...
@section('content')
    ...
    <form method="POST" action="{{ route('application.update', ['application' => $application]) }}" id="application_form" enctype="multipart/form-data">
        ...
        @foreach (range(1, $application->projectcall->number_of_laboratories) as $index => $iteration)
            @include('forms.selectorother', [
                ...
                'value' => old(
                    ...
                    count($application->laboratories) >= $iteration ? $application->laboratories[$index]->id : 'none'
                ),
                'fields' => [
                    [
                        'type' => 'textinput',
                    ...
                    [
                        ...
                        'value' => old(
                            ...
                            count($application->laboratories) >= $iteration ? $application->laboratories[$index]->pivot->contact_name : ''
                        ),
                    ],
                ],
            ])
        ...
        @include('forms.select', [
            ...
            'help' => __('fields.application.study_fields_help', ['max' => $application->projectcall->number_of_study_fields]),
        ...
        @foreach (range(1, $application->projectcall->number_of_keywords) as $index => $iteration)
            @include('forms.textinput', [
                ...
                'value' => old(
                    'keyword_' . $iteration,
                    count(is_array($application->keywords) ? $application->keywords : json_decode($application->keywords)) >= $iteration
        ...
        @foreach (['application', 'financial'] as $key)
            @if (!empty($application->projectcall->{$key . '_form_filepath'}))
                @include('forms.filedownload', [
                    'text' => __('fields.application.template.prefix.' . $key) . ' ' . $application->projectcall->type->label_short,
        ...
        <div class="form-group row">
            <div class="col-sm-12 text-center">
                ...
                <a href="{{ route('application.submit', ['application' => $application]) }}" class="btn btn-success submission-link">
                    @svg('solid/check')
