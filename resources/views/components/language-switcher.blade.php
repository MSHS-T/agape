@php
    $localeData = config('filament-language-switch.locales');
...
<button type="button" x-data="language_switcher" x-on:click="next"
    ...
    <div class="w-full flex items-center justify-start">
        <x-dynamic-component :component="'flag-1x1-' . (!blank($localeData[$locale]['flag_code']) ? $localeData[$locale]['flag_code'] : 'un')"
            class="flex-shrink-0 w-4 h-4 mr-2 group-hover:text-white group-focus:text-white text-primary-500"
            style="border-radius: 0.25rem" />
        {{ $localeData[$locale]['native'] }}
    </div>
</button>
@push('scripts')
    <script>
