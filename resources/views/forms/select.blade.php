...
<div class="form-group row">
    ...
    <div class="col-9">
        <select class="form-control" id="{{$id}}" name="{{$name}}">
            <option value="none" @if (!$allowNone) disabled @endif @if(empty($value)) selected @endif>{{__('fields.none')}}</option>
            @foreach($allowedValues as $v)
            @php
            $valueVal = $v->{$valueField};
            $displayVal = $v->{$displayField};
            @endphp
            <option value="{{ $valueVal }}" @if($value==$valueVal) selected @endif>{{ $displayVal }}</option>
            @endforeach
        </select>
        @if(isset($help))
