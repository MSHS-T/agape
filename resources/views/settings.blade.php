    ...
    <form method="POST" action="{{ route('settings.update') }}" id="settings_form" enctype="multipart/form-data">
        ...
        @foreach (range(3, 0) as $index)
            @include('forms.textinput', [
                ...
                'value' => old('notation_grid_' . $index . '_grade', json_decode($settings->notation_grid, true)[$index]['grade']),
                'tabindex' => ++$tabindex,
            ])
            @include('forms.textinput', [
                'name' => 'notation_grid_' . $index . '_details',
                'label' => __('fields.setting.notation_details'),
                'value' => old('notation_grid_' . $index . '_details', json_decode($settings->notation_grid, true)[$index]['details']),
                'tabindex' => ++$tabindex,
            ])
        @endforeach
