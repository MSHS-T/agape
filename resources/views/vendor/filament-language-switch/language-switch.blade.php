<x-filament::dropdown teleport placement="bottom-end" class="fi-dropdown fi-user-menu">
    ...
    <x-slot name="trigger" class="">
        @if (config('filament-language-switch.flag'))
            @php($locale = config('filament-language-switch.locales')[app()->getLocale()])
            <div
                class="flex items-center justify-center w-8 h-8 font-semibold text-sm text-white rounded-full language-switch-trigger bg-primary-500 dark:text-primary-500 dark:bg-gray-900 ring-1 ring-inset ring-gray-950/10 dark:ring-white/20 overflow-hidden">
                <x-dynamic-component :component="'flag-1x1-' . (!blank($locale['flag_code']) ? $locale['flag_code'] : 'un')"
                    class="flex-shrink-0 w-8 h-8 group-hover:text-white group-focus:text-white text-primary-500"
                    style="border-radius: 0.25rem" />
            </div>
        @else
            <div
                class="flex items-center justify-center w-8 h-8 font-semibold text-sm text-white rounded-full language-switch-trigger bg-primary-500 dark:text-primary-500 dark:bg-gray-900 ring-2 ring-inset ring-gray-950/10 dark:ring-white/20">
